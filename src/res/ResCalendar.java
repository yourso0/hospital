...
public class ResCalendar extends JDialog {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ResCalendar dialog = new ResCalendar();
	...
	protected void ShowCalendar() {
		...
		for(int i=1 ; i<= lastDay ; i++) {
			...
			panelCalendar.add(btn);
			 // 현재 날짜 이전의 날짜를 비활성화
	        if (year < curYear || (year == curYear && month + 1 < curMonth) || (year == curYear && month + 1 == curMonth && i < curDate)) {
